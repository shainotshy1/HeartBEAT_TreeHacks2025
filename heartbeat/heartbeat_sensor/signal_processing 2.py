...
class SignalProcessor:
    """Processes the raw heartbeat signal to extract features like BPM and variability."""
    ...
    def compute_hrv(self) -> float:
        """
        Calculates heart rate variability using RMSSD 
        ...
        rr_intervals = np.array(self.bpm_history)
    ...
    def update_bpm(self, bpm: float):
        """
        Updates the BPM history.
        
        Args:
            bpm: New BPM value to add
        """
    ...
    def get_signal_quality(self) -> float:
        """
        Estimates the quality of the signal based on noise level and stability.
        
        Returns:
            Signal quality score between 0 and 1
        """
        if len(self.raw_signal) < 50:
            return 0.0
            
        # Calculate signal-to-noise ratio and stability
        signal_std = np.std(self.raw_signal)
        signal_range = np.ptp(self.raw_signal)
        
        # Normalize and combine metrics
        quality_score = 1.0 - min(1.0, (signal_std / signal_range))
